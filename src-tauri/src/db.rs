...
async fn seed_database_if_empty(pool: &SqlitePool) -> Result<(), sqlx::Error> {
    ...
    if count.0 == 0 {
        let mock_data = vec![
            ("q1", "Mathematics", "Calculus", 4, "Find the derivative of f(x) with respect to x, and determine all critical points in the interval [0, 2π].", "f(x) = 3x³ - 2sin(x) + e^(2x)", false),
            ...
            ("q4", "Mathematics", "Statistics", 5, "Given a normal distribution X ~ N(μ, σ²), find the probability P(X > 72) given that μ = 65 and σ = 8.", "P(X > 72) = P(Z > (72 - μ) / σ)", false),
        ...
        for (id, subject, subtopic, marks, content, math_snippet, is_code) in mock_data {
            sqlx::query(
                r#"
